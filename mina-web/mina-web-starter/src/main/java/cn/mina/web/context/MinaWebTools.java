...
import cn.mina.web.token.jwt.JwtTokenHelper;
...
public class MinaWebTools {
    ...
    private static Logger logger = LoggerFactory.getLogger(MinaWebTools.class);
    ...
    public static class token {
        ...
        public static <T extends MinaWebContext> String generate(T t) {
            switch (properties.token.getType()) {
                case "jwt":
                    return JwtTokenHelper.encode(t, Integer.valueOf(properties.token.getExpireTime()), properties.token.getSecret());
                case "redis":
                    // TODO: 2022/7/21
                    return JwtTokenHelper.encode(t, Integer.valueOf(properties.token.getExpireTime()), properties.token.getSecret());
                default:
                    throw new MinaGlobalException(GlobalErrorCode.ERROR_ILLEGAL_CONFIG.customMassage("不支持的token管理类型"));
            }
        }
    }
    ...
    public static class context {
        ...
        public static <T extends MinaWebContext> T getContextByClazz(Class<T> clazz) {
            if (ClassUtil.isAssignable(MinaWebContext.class, clazz)) {
